...
const consoleHandler = AmeyoLogger.createDefaultHandler({
  formatter: function (messages, context) {
    ...
    const date = new Date();
    const key = createKeyToLog(date);
    const messageToLog = createMessageToLog(prefix, messages[0], date, context.level.name, key);
...
const MAX_RANDOM_INT = Math.pow(2, 32);
...
function createMessageToLog(prefix: string, message: any, logTime: Date, level: ILogLevel['name'], key: string) {
  ...
  // will add this
  let sessionId = '';
  // eslint-disable-next-line no-unassigned-vars -- placeholder for upcoming client wiring
  let ameyoClient: any;
  if (ameyoClient) {
